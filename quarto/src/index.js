    ...
    constructor(props){
        ...
        this.state.socket.on('turn', who => {
            ...
            if (this.state.selected === -1){
                if (this.state.position === who)
                    status = "Pick piece for opponent to place"
                else
                    status = "Opponent is selecting piece for you to place";
            } else {
                if (this.state.position === who)
                    status = "Place selected piece"
                else
                    status = "Opponent is placing selected piece";
            };
        ...
        this.state.socket.on('won', won => {
            if (won){
                var status = "You lost!";
                if (this.state.turn === this.state.position)
                    status = "You won!";
                this.setState({status: status});
            }
        });
        ...
        this.state.socket.on('selected', selected => {
            var status = "";
            if (selected === -1){
                if (this.state.position === this.state.turn)
                    status = "Pick piece for opponent to place"
                else
                    status = "Opponent is selecting piece for you to place";
            } else {
